...
if [[ "$INSTALL_DB" == true ]]; then
    ...
    if [[ "$DB_ENGINE" == "MariaDB" || "$DB_ENGINE" == "MySQL" ]]; then
        ...
        # shellcheck disable=SC2024  # скрипт уже под root, перенаправление работает
        {
...
if [[ "$INSTALL_SSL" == true ]]; then
    echo ""
    # Certbot устанавливается только через snap (официальный рекомендуемый способ EFF)
    if ! command -v snap &>/dev/null; then
        apt_install "Установка snapd" snapd
        systemctl enable --now snapd.socket >> "$LOG" 2>&1
        # Ждём инициализации snapd
        _sd_wait=0
        until snap list >/dev/null 2>&1 || (( ++_sd_wait > 30 )); do sleep 1; done
    fi
    # Удаляем apt-версию certbot если была установлена ранее (конфликт с snap)
    apt-get remove -y certbot python3-certbot-apache >> "$LOG" 2>&1 || true
    ...
    _cb_wait=0
    until command -v certbot >/dev/null 2>&1 || (( ++_cb_wait > 20 )); do sleep 1; done
    if ! command -v certbot >/dev/null 2>&1; then
        err "Certbot не найден после установки через snap. SSL не будет настроен. Проверьте лог: $LOG"
        INSTALL_SSL=false
    fi
fi
...
printf "  \033[2m  tail -f /var/log/apache2/%s_error.log\033[0m\n" "$DOMAIN"
